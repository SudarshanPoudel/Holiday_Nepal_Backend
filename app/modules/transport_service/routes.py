...
@router.post("/")
async def create_transport_service(transport_service: TransportServiceCreate, db: AsyncSession = Depends(get_db)):
    try:
...
async def get_transport_service(transport_service_id: int, db: AsyncSession = Depends(get_db)):
    try:
        controller = TransportServiceController(db)
...
async def update_transport_service(transport_service_id: int, transport_service: TransportServiceCreate, db: AsyncSession = Depends(get_db)):
    try:
        controller = TransportServiceController(db)
...
async def delete_transport_service(transport_service_id: int, db: AsyncSession = Depends(get_db)):
    try:
        controller = TransportServiceController(db)
